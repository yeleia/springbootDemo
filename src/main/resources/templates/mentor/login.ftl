...
<head>
	...
	<script>
		function sub() {
            ...
            var mentorPassword = $("#mentorPassword").val();
            if (mentorName == null || mentorName == "") {
                alert("请输入登录账户")
            } else if (mentorPassword == null || mentorPassword == "") {
                alert("请输入登录密码")
            } else {
                $.ajax({
					url:"${request.contextPath}/menitor/ajaxLogin",
					type:"post",
					dataType:"json",
					data:{loginName:mentorName,loginPass:mentorPassword},
					success:function (result) {
					   if(result.status!=200){
                           alert(result.message)
					   }else {
					       alert(result.message)
                           location.href ="${request.contextPath}/test";
					   }
                    }
				})
...
<div id="login-window">
	...
	<div class="input-group m-b-20">
		<span class="input-group-addon"><i class="zmdi zmdi-account"></i></span>
		<div class="fg-line">
			<input id="mentorName" type="text" class="form-control" name="username" placeholder="帐号" required autofocus >
		</div>
	</div>
	<div class="input-group m-b-20">
		<span class="input-group-addon"><i class="zmdi zmdi-male"></i></span>
		<div class="fg-line">
			<input id="mentorPassword" type="password" class="form-control" name="password" placeholder="密码" required >
		</div>
	</div>
	<div class="clearfix">
	</div>
    ...
    <div class="input-group m-b-20">
        <span class="input-group-addon"><#--<i class="zmdi zmdi-account"></i>--></span>
        <div class="fg-line" style="align-content: center">
            <center><a  href="${request.contextPath}/toStudentLogin"><button>学生登录</button></a></center>
        </div>
    </div>
</div>
<div>

    ...
    <div style="margin-left: 50px">
       <#--<button type="button" class="btn btn-success" data-toggle="modal" data-target="#myModal">上传论文</button>-->
        <table class="table" style="margin-top: 50px">
            ...
            <#list history as history>
            <tr>
             <td><a href="${request.contextPath}/static/upload/${history.paper}" target="_blank"><button>预览</button></a>&nbsp;&nbsp;<a href=""><button>下载</button></a></td>
             <td>${history.ploadtime?string("yyyy-MM-dd HH:mm:ss")}</td>
                <#if history.pstatus=="0">
                <td>未阅</td>
                <#else>
                <td>已阅</td>
                </#if>
